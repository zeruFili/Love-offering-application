...
class _TransactionHistoryPageState extends State<TransactionHistoryPage>
  ...
  @override
  void dispose() {
    // Mark transactions as viewed only when leaving the page
    if (!_hasMarkedAsViewed) {
      _markTransactionsAsViewed();
    }
  ...
  Future<void> _markTransactionsAsViewed() async {
    if (_hasMarkedAsViewed) return;

    _hasMarkedAsViewed = true;

    // Mark supporter transactions as viewed
    await _transactionController.fetchTransactionsBySupporter();
    await _transactionController.markAllCurrentTransactionsAsViewed();

    // Mark artist transactions as viewed
    await _transactionController.fetchTransactionsByArtist();
    await _transactionController.markAllCurrentTransactionsAsViewed();
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            // Mark as viewed when navigating back
            if (!_hasMarkedAsViewed) {
              _markTransactionsAsViewed();
            }
            Get.back();
